...
export type CardioManualSessionFormValues = {
    ...
    cardioEnvironment: Exclude<WorkoutCardioEnvironment, null>;
...
export type BuildCardioManualSessionPayloadResult =
...
function buildSessionType(
    ...
    const environmentLabel = cardioEnvironment === "indoor" ? "Indoor" : "Outdoor";
    const activityLabel = activityType === "running" ? "Running" : "Walking";
...
function hasAtLeastOneMetric(values: CardioManualSessionFormValues): boolean {
    const metricValues = [
...
export function buildCardioManualSessionFormFromSession(
    session: WorkoutSession,
    date: ISODate
): CardioManualSessionFormValues {
    const cardioMetrics = session.cardioMetrics ?? null;

    return {
        date,
        activityType: session.activityType,
        cardioEnvironment: session.cardioEnvironment === "indoor" ? "indoor" : "outdoor",
        ...
        avgSpeedKmh: formatOptionalNumber(cardioMetrics?.avgSpeedKmh),
        maxSpeedKmh: formatOptionalNumber(cardioMetrics?.maxSpeedKmh),

        cadenceRpm: formatOptionalNumber(session.cadenceRpm),
        strideLengthM: formatOptionalNumber(cardioMetrics?.strideLengthM),
...
export function buildCardioManualSessionPayload(
    values: CardioManualSessionFormValues
): BuildCardioManualSessionPayloadResult {
    ...
    const payload: CreateSessionBody = {
        type: buildSessionType(values.activityType, values.cardioEnvironment),
