// /src/features/health/cardio/screens/CardioManualSessionScreen.tsx
...
import {
    buildCardioManualSessionFormFromSession,
    buildCardioManualSessionPayload,
    type CardioManualSessionFormValues,
} from "@/src/features/health/cardio/utils/buildCardioManualSessionPayload";
...
function buildInitialForm(date: ISODate): CardioManualSessionFormValues {
    return {
        date,
...
export function CardioManualSessionScreen() {
    ...
    const [form, setForm] = React.useState<CardioManualSessionFormValues>(() =>
    ...
    React.useEffect(() => {
        ...
        if (!session || !isManualCardioEditable(session) || didHydrateEditForm.current) {
            return;
        }

        setForm(buildCardioManualSessionFormFromSession(session, initialDate));
        didHydrateEditForm.current = true;
    }, [details.session, initialDate, isEditMode]);

    function updateField<TKey extends keyof CardioManualSessionFormValues>(
        key: TKey,
        value: CardioManualSessionFormValues[TKey]
    ) {
        setForm((current) => ({
            ...current,
            [key]: value,
    ...
    async function handleSave() {
        ...
        const result = buildCardioManualSessionPayload(form);
        if (!result.ok) {
        ...
        try {
            ...
            setError(
                toErrorMessage(
                    err,
                    isEditMode
                        ? "No se pudo actualizar la sesión manual de cardio."
                        : "No se pudo guardar la sesión manual de cardio."
                )
            );
        } finally {
            setSubmitting(false);
        ...
        return (
            ...
            >
                <SectionCard
                    ...
                    subtitle="No encontramos una sesión manual de cardio para este identificador."
                >
    ...
    if (isEditMode && !isManualCardioEditable(details.session)) {
        return (
            <ScrollView
                style={{ flex: 1, backgroundColor: colors.background }}
                contentContainerStyle={{ padding: 16, gap: 16, paddingBottom: 32 }}
            >
                <SectionCard
                    ...
                    subtitle="Solo las sesiones manuales de cardio pueden modificarse desde este formulario."
                >
                    <Text style={{ color: colors.mutedText, lineHeight: 20 }}>
        ...
        >
            <View style={{ gap: 4 }}>
                <Text style={{ fontSize: 24, fontWeight: "800", color: colors.text }}>
                    {isEditMode ? "Editar sesión de cardio" : "Captura manual de cardio"}
                </Text>
                <Text style={{ color: colors.mutedText }}>
                    {isEditMode
                        ? "Ajusta la sesión manual si necesitas corregir datos capturados manualmente."
                        : "Usa este fallback para capturar caminadora, caminata o carrera cuando no haya datos en HealthKit / Health Connect para este día."}
                </Text>
            </View>
            ...
            >
                ...
                <ToggleGroup<CardioActivityType>
                    label="Actividad"
                ...
                <ToggleGroup<"outdoor" | "indoor">
                    label="Ambiente"
                    value={form.cardioEnvironment}
                    onChange={(next) => updateField("cardioEnvironment", next)}
                    options={[
...
export default CardioManualSessionScreen;
