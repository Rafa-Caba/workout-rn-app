...
function Button(props: { title: string; onPress: () => void; tone?: "primary" | "danger" | "neutral"; disabled?: boolean }) {
    ...
    const color = tone === "primary" ? colors.primaryText : colors.text;
    ...
    return (
        ...
        >
            <Text style={{ fontWeight: "900", color, textAlign: "center" }}>{props.title}</Text>
        </Pressable>
    );
}
    ...
    return (
        <View style={{ gap: 12 }}>
            <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center" }}>
                <Text style={{ fontSize: 16, fontWeight: "900", color: colors.text }}>Ejercicios del día</Text>
                <Button title="Añadir ejercicio" onPress={add} tone="primary" />
            </View>

            {(value ?? []).length === 0 ? <Text style={{ color: colors.mutedText }}>No hay ejercicios todavía.</Text> : null}
            ...
            {(value ?? []).map((ex, idx) => (
                ...
                >
                    <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center", gap: 12 }}>
                        <Text style={{ fontWeight: "900", color: colors.text }}>Ejercicio #{idx + 1}</Text>
